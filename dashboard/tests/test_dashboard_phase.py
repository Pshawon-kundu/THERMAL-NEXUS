...
def test_v4_live_hardware_tables_created(tmp_path: Path) -> None:
    ...
    assert {"gps_readings", "stm_samples", "receiver_events", "serial_status"} <= tables
...
def test_runtime_data_mode_stale_when_connected_but_stale(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from host.dashboard import runtime_status as status
    ...
    stale_live_packet = status.PacketSnapshot(
        experiment_id=None,
        data_source_type="PROJECT_COLLECTED",
        source_type="live_serial",
        received_at=1.0,
        age_seconds=60.0,
    )
    config = {"database_path": "unused.db", "freshness_timeout_seconds": 10}
    monkeypatch.setattr(status, "get_serial_snapshot", lambda _config: connected)
    monkeypatch.setattr(
        status, "get_latest_packet_snapshot", lambda _path: stale_live_packet
    )
    assert status.get_runtime_snapshot(config).mode == "STALE"
