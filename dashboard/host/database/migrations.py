...
Supports a v1 → v2 → v3 → v4 upgrade path:

* v2 adds the four hardware-phase tables (`hardware_runs`,
  `measurement_traces`, `bom_items`, `physical_measurements`) and a
  `source_event_id` column on `alerts`.
* v3 adds MQTT provenance and reader metadata columns.
* v4 adds the live-hardware tables (`gps_readings`, `stm_samples`,
  `receiver_events`, `serial_status`) and their indexes.

Fresh databases skip straight to the latest schema version.
"""
...
# v4 only adds new tables + indexes (all CREATE IF NOT EXISTS), so no ALTER
# statements are required. Keeping the list documents the upgrade intent.
V3_TO_V4_STATEMENTS: list[str] = []

# v5 adds the telemetry_readings table for unified binary telemetry packets.
V4_TO_V5_STATEMENTS: list[str] = []
...
def _boot_latest_fresh(connection: sqlite3.Connection) -> None:
...
def _upgrade_v3_to_v4(connection: sqlite3.Connection) -> None:
    """Add live-hardware tables (GPS, STM, events, serial status)."""
...
def _upgrade_v4_to_v5(connection: sqlite3.Connection) -> None:
    """Add telemetry_readings table for unified binary telemetry."""

    _apply_statements(connection, V4_TO_V5_STATEMENTS)
    _apply_statements(connection, DDL)
    _apply_statements(connection, INDEXES)
    connection.execute(
...
def initialize_database(database_path: Path = DEFAULT_DATABASE_PATH) -> int:
    ...
    with connect(database_path) as connection:
        ...
        if current == 3 and current < SCHEMA_VERSION:
            ...
            current = 4
        if current == 4 and current < SCHEMA_VERSION:
